  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(item,index) in cartInfoList" :key="index">
          ...
          <li class="cart-list-con5">
            <a href="javascript:void(0)" class="mins" @click="handler(item,'-')">-</a>
            <input :disabled="true" autocomplete="off" type="text" :value="item.skuNum" minnum="1" class="itxt">
            <a href="javascript:void(0)" class="plus" @click="handler(item,'+')">+</a>
          </li>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChedked" @change="changeAllChecked">
        <span>全选</span>
      </div>
      <div class="option">
...
import { debounce } from '@/assets/js/common'
  export default {
    ...
    computed:{
      ...
      totalNum(){
        ...
        this.cartInfoList.forEach(item=>{
          if(item.isChecked==1)sum+=item.skuNum
        })
        return sum
      },
      totalPrice(){
    ...
    methods:{
      getData(){
        this.$store.dispatch('shopcart/reqCarList')
      },
      handler: debounce(async function(item,type){
        let change
        if(type=='-'){
          change = item.skuNum<=0?0:-1
        }
        if(type=='+'){
          change = 1
        }
      ...
      async deleteCartById(item){
        try{
          await this.$store.dispatch('shopcart/deleteCartBySkuId',item.skuId)
          this.getData()
        }catch(error){
          console.log(error.message)
        }
      },
      async updateChecked(event,item){
