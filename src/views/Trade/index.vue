  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item,index) in userAddress" :key="index">
        <span class="username" :class="{selected:currentIndex == index}">{{item.name}}</span>
        <p @click="changeSelect(index)">
          <span class="s1">{{item.address}}</span>
          <span class="s2">{{item.phone}}</span>
          <span class="s3" v-if="item.default">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        <br/>
        收货人：<span>{{currentInfo.name}}</span>
        <br/>
        <span> 联系方式：{{currentInfo.phone}}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <a class="subBtn" @click="submitOrder" style=":hover{color:#fff}">提交订单</a>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods:{
      ...
      async submitOrder(){
        ...
        let data = {
            "consignee": this.currentInfo.name,
            "consigneeTel": this.currentInfo.phone,
            "deliveryAddress": this.currentInfo.address,
            "paymentWay": "ONLINE",
